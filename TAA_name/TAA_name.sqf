...
//#define _DEBUG_HUD true
	...
	_frameNo 	= diag_frameNo;
		...
		if( TAA_name_HUD_town)then {[] call fnc_town_show;};
		...
		_insigna			= TAA_name_HUD_insigna_color;
			{
				if (!(terrainIntersect [(eyePos _x), (eyePos player)]) && {!(lineIntersects [(eyePos player), (eyePos _x), player, _x])} && TAA_visibility) then {
						_Display_x	= false;
...
fnc_assign_key={
 [_this] spawn{
		_binded = _this select 0;
		_result = [ format[localize "STR_TAA_KEY_SURE",keyName _binded],"Tag name system - key", true, true,(findDisplay 10000) ] call BIS_fnc_guiMessage; // Arma 3
		if(_result)then{
				TAA_name_HUD_key = _binded;
				hint format["You binded a new key %1 to active tag name system",keyName _binded];
			}else{
				hint format["You cancelled the bind %1",keyName TAA_name_HUD_key];
			};
...
fnc_town_show={
			...
			disableSerialization;
			_towns = nearestLocations  [getPosATL player, ["NameVillage","NameCity","NameCityCapital"],TAA_name_HUD_distance_town];
			if(count _towns > 0)then{
				_nearest_town 		= _towns select 0;
				_TownPosition		= position (_nearest_town);
				_ScreenPos 			= worldToScreen _TownPosition;

				if(count _ScreenPos >= 1 && _TownPosition distance player < TAA_name_HUD_distance_town)then{
					_ScreenX 		=	_ScreenPos select 0;
					_ScreenY 		=	_ScreenPos select 1;
						1006 cutRsc ["TAA_dyn_name","PLAIN"];
						_ui 		= uiNameSpace getVariable "TAA_dyn_name";
						_dyn_name 	= _ui displayCtrl 7000;
						_formatTown = format["<t size='1.2' align='left' color='#FDFDFD'>%1(%2 m)</t>",(text _nearest_town),floor (player distance _TownPosition)];
						_dyn_name ctrlSetStructuredText parseText _formatTown;
						_dyn_name ctrlSetPosition [0.5, - 0.20, 0.5, 0.5];
						_dyn_name ctrlCommit 0;
						_dyn_name ctrlShow true;
				};
			};
...
fnc_format_vehicle_air={
	...
	_passengerImg="<img size='0.6' align='left' image='\a3\ui_f\data\IGUI\Cfg\Actions\getincargo_ca.paa'/>";
	...
	if(_locked)then{
		...
		_passengers = [];
		{
			...
			switch(_Role select 0)do{
				...
				case "Cargo":
				{
					_passengers set [count _passengers,name _x ];
				};
			};
		}forEach _crews;
		...
		if(count _passengers > 0)then{
			{
				_Formated = format ["%4%5%6%1%2%3",_Color, _x, _END,_Formated,_passengerImg,_SPACE];
			}foreach _passengers;
		};
...
fnc_Details_vehicle = {
	private ["_isPlayer","_isEnnemy","_driver","_gunner","_commander","_driverStat","_gunnerStat","_commanderStat","_locked","_passengers","_Role","_Details"];
_target 		= _this select 0;
	_isPlayer 		= false;
	_isEnnemy		= true;
	...
	_Details = [_isPlayer,_isEnnemy,_driverStat,_gunnerStat,_commanderStat,_locked,_passengers ];
_Details
};
fnc_Format_vehicle = {
	private ["_Driver","_Gunner","_commander","_locked","_Color","_Size","_refresh","_Posx","_PosY","_ColorEmpty","_ColorVeh","_END","_Formated","_SPACE","_Label","_EmptyConf","_EmptyPos","_DriverImg","_GunnerImg","_commanderImg","_passengers","_Target","_Posy"];
_Driver 	= _this select 0;
	...
	if(count _passengers > 0)then{
		{
			_Formated = format ["%4%5%6%1%2%3",_Color, _x, _END,_Formated,_SPACE];
		}foreach _passengers;
	};
...
private["_oldTarget","_distance_Infantry","_distance_Land_Vehicle","_distance_Air_Vehicle","_AIDisplayName","_PlayerDisplayName","_DisplayEnnemy","_DisplayInVehicle","_ColorAiFriendly","_ColorPlayer","_ColorEnnemy","_ColorAIEnnemy","_Size","_Posx","_Posy","_initTime","_frameNo","_target","_Details","_isPlayer","_isEnnemy","_rank","_Driver","_gunner","_commander","_Locked"];
...
//--- Capture what is aiming the player ---//
	currentTarget = cursorTarget;
	_target = currentTarget;
	if (!(terrainIntersect [(eyePos _target), (eyePos player)]) && {!(lineIntersects [(eyePos player), (eyePos _target), player, _target])} && TAA_visibility) then {
	...
	/******************************************************************************************************/
		if((player distance _target) < _distance_Land_Vehicle && player == vehicle player)then{
			if ((_target isKindOf "Car" || _target isKindOf "Motorcycle" || _target isKindOf "boat" || _target isKindOf "Tank") && player == vehicle player && ((count crew _target) > 0))then{
					...
					_Passengers 	= _Details select 6;
	//--- AI ennemy ---//
					switch(true)do{
						case ((_AIDisplayName) &&(!(_isPlayer))&&(_isEnnemy)&&(_DisplayEnnemy)):
						{
								[_Driver,_gunner,_commander,_target,_Locked,_ColorAIEnnemy,_Size,_refresh,_Posx,_Posy,_Passengers] call fnc_Format_vehicle;
						};
	//--- AI friendly ---//
						case ((_AIDisplayName)&&(!(_isPlayer))&&(!(_isEnnemy))):
						{
								[_Driver, _gunner, _commander, _target, _Locked, _ColorAiFriendly, _Size,_refresh,_Posx,_Posy,_Passengers] call fnc_Format_vehicle;
						};
	//--- Player ennemy ---//
						case ((_isPlayer)&& (_isEnnemy) &&(_DisplayEnnemy)&&(_PlayerDisplayName)):
						{
								[_Driver, _gunner, _commander, _target, _Locked, _ColorEnnemy, _Size,_refresh,_Posx,_Posy,_Passengers] call fnc_Format_vehicle;
						};
	//--- Player friendly ---//
						case ((_isPlayer)&& !(_isEnnemy)&&(_PlayerDisplayName)):
						{
								[_Driver, _gunner, _commander, _target, _Locked, _ColorPlayer, _Size,_refresh,_Posx,_Posy,_Passengers] call fnc_Format_vehicle;
						};
					};
				};
			};
	...
	//--- AI friendly ---//
								case ((_AIDisplayName) && !(_isPlayer)&& !(_isEnnemy)):
								{
										[_Locked, _ColorAIfriendly, _Size,_refresh,1.2,_Posy] call fnc_Format_vehicle_air;
								};
	//--- Player friendly ---//
								case ((_isPlayer)&& !(_isEnnemy)&&(_PlayerDisplayName)):
								{
										[_Locked, _ColorPlayer, _Size,_refresh,1.2,_Posy] call fnc_Format_vehicle_air;
								};
							};
						};
				if ((_target isKindOf "Car" || _target isKindOf "Motorcycle" || _target isKindOf "boat" || _target isKindOf "Tank"))then{
					switch(true)do{
						case ((_AIDisplayName) && !(_isPlayer)&& (_isEnnemy) &&(_DisplayEnnemy)):
						{
								[_Driver,_gunner,_commander,_target,_Locked,_ColorAIEnnemy,_Size,_refresh,1.2,_Posy,_Passengers] call fnc_Format_vehicle;
						};
	//--- AI friendly ---//
						case ((_AIDisplayName)&&!(_isPlayer)&&!(_isEnnemy)):
						{
								[_Driver, _gunner, _commander, _target, _Locked, _ColorAiFriendly, _Size,_refresh,1.2,_Posy,_Passengers] call fnc_Format_vehicle;
						};
	//--- Player ennemy ---//
						case ((_isPlayer)&&(_isEnnemy)&&(_DisplayEnnemy)&&(_PlayerDisplayName)):
						{
								[_Driver, _gunner, _commander, _target, _Locked, _ColorEnnemy, _Size,_refresh,1.2,_Posy,_Passengers] call fnc_Format_vehicle;
						};
	//--- Player friendly ---//
						case ((_isPlayer)&& !(_isEnnemy)&&(_PlayerDisplayName)):
						{
								[_Driver, _gunner, _commander, _target, _Locked, _ColorPlayer, _Size,_refresh,1.2,_Posy,_Passengers] call fnc_Format_vehicle;
						};
					};
				};
...
//optional also for more randomization:
		...
		sleep _refresh;
		if(TAA_CURSOR_MUTEX&&!TAA_name_Show && (currentTarget != cursorTarget && vehicle player == player))then{[] spawn fnc_cursor_hide;};
	};
/****************************************************************************************************/
/****************************************** End Loops ***********************************************/
/****************************************************************************************************/
